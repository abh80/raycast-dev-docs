...
export async function getCachedPage(providerId: string, version: string, url: string): Promise<DocPage | null> {
  return readJson<DocPage>(pagePath(providerId, version, url));
}

export async function setCachedPage(providerId: string, version: string, url: string, page: DocPage): Promise<void> {
